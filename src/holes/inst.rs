...
impl Instantiate {
    ...
    /// Replace the SPP assigned to `hole` (creating the entry if absent).
    /// Used by the CEGIS loop to swap in a new candidate without rebuilding
    /// the inner automaton (which holds the lazy derivative caches).
    pub fn set_hole(&mut self, hole: Hole, spp: spp::SPP) {
